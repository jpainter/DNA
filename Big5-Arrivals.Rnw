...
opts_chunk$set( echo=TRUE, cache=FALSE, results='asis',
...
us = merge(us, us.tb, by.x=1, by.y=1 )[,c(1,2,4,8)]
aus = merge(aus, aus.tb, by.x=1, by.y=1 )[,c(1,2,4,8)]
nz = merge(nz, nz.tb, by.x=1, by.y=1 )[,c(1,2,4,8)]
ca = merge(ca, ca.tb, by.x=1, by.y=1 )[,c(1,2,4,8)]
uk = merge(uk, uk.tb, by.x=1, by.y=1 )[,c(1,2,4,8)]
...
top.arrivals = cbind(aus.top[,], nz.top[,], us.top[,], ca.top[,], uk.top[,])
# print top 25
