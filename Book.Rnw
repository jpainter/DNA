...
# Incoming: data with key attributes aggregated by country and destination state ----
  library(plyr)
  countryToState = ddply(.data = edn, .(country.code, state.abb), 
                         summarize, 
...
# calculate routes ----
  ...
  rts <- gcIntermediate( incoming[ ,c('CountryLong', 'CountryLat')], 
                         ...
                         30, breakAtDateLine=TRUE, addStartEnd=TRUE, sp=TRUE) 
...
g= ggplot() +
  ...
  geom_polygon(aes(long,lat, group=group), size = 0.2, fill="White", colour = "grey35", alpha=.1, data=states) +  
  theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank(),  
       axis.ticks = element_blank(), 
       axis.title.x = element_blank(), axis.title.y = element_blank(), 
       axis.text.x = element_blank(), axis.text.y = element_blank()
...
routes = geom_line(aes(long,lat, 
                       group=group), # removed colour=AlienType, 
#                                   type=Class), 
                   alpha=.1, 
                   lineend="round", 
                   lwd=.01,  
