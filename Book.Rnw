...
# save
  save(country.lat.long, file="country.lat.long.RData")  load("country.lat.long.RData")
...
<<'incoming-map-routes', eval=FALSE>>=     
  ...
  load("country.lat.long.rda")

# Incoming: data with key attributes aggregated by country and destination state ----
library(plyr)
countryToState = mutate(data = incoming, .(Country, state),
                        count = nrow(),
                        StartLong= max(StartLong), 
                        StartLat = max(StartLat),
                        stateLong = max(stateLong),
                        stateLat = max(stateLat)
                        )
                        
#  Insert lat/long data for place of origin 
  incoming = merge(countryToState, 
                   country.lat.long, by.x="country.code", by.y="FIPS")
...
#  Insert lat/long data for destination state 
 incoming = merge(countryToState, state.codes, by.x="abb", by.y="abb")

# save
  save(incoming, "incoming.rda")
...
# remove unneeded objects
  rm(state.codes, country.lat.long, edn)
