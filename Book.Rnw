...
<<'plot-map', cache=TRUE, fig.cap='Map of EDN Arrivals', eval=TRUE>>=
#### retreive map data ----
...
routes = geom_line(aes(long.recenter,lat, group=group.regroup, colour=AlienType, type=Class), alpha=.1, lineend="round", lwd=.1, 
                   data= gcircles.rg)
g + routes
