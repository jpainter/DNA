...
<<'incoming-refugees-region', fig.height=7, fig.width=7, fig.cap='Refugee Resettlement by Nationality', cache=TRUE>>=
require(XLConnect)
...
<<'Heatmap', eval=TRUE, fig.height=7, fig.width=7, fig.cap="Heatmap of Refugee Arrivals">>=
# Load spreadsheet
require(XLConnect)
wb <- loadWorkbook(
...
<<'incoming-map-data', fig.cap='Map of EDN Arrivals',eval=false>>=
startTime = date()
...
library(rgdal)
library(maptools) 
states.shp <- importShapefile("C:/Users/bzp3/Desktop/R/Maps/US/statesp020.shp") # from http://nationalatlas.gov/atlasftp.html#statep0
str(states.shp@data)  # @data to primary variables (excludes polygon stuff)
...
<<'recalculate-map-data', fig.cap='Map of EDN Arrivals'>>=
